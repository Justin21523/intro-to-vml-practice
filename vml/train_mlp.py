...
from .torch_models import TorchMLPArch, TorchMLPClassifier, infer_num_classes, num_trainable_params
...
class MLPTrainConfig:
    ...
    # input pipeline
    channels: str = "grayscale"
    dtype: str = "float32"
    shape_mode: str = "resize"
...
def train_mlp(
    ...
    *,
    ...
    val_loader = (
        make_dataloader(val_known, pipeline_config=eval_pipeline, expected_shape=expected_shape, is_train=False, data=data_eval)
        if val_known
        else None
    )
    test_loader = (
        make_dataloader(test_known, pipeline_config=eval_pipeline, expected_shape=expected_shape, is_train=False, data=data_eval)
        if test_known
        else None
    )

    arch = TorchMLPArch(hidden_sizes=tuple(int(x) for x in config.hidden_sizes), dropout=float(config.dropout))
...
def train_mlp_from_splits(
    *,
    ...
    model, metrics = train_mlp(train_records, val_records, test_records, config=config)
    ...
    meta = {
        ...
        "arch": {"hidden_sizes": list(config.hidden_sizes), "dropout": float(config.dropout)},
